...
public class Helicopter2 extends Component {
	...
	public static void computeTarget(
			...
			@InOut("hHSearch") OutWrapper<Boolean> hHSearch,
		...
		if(hFFCreationTime != 0.0){
			...
			if (hFFTargetPos.value != 0.0) inaccuracy = Math.max( Math.abs(hFFPos  - hFFPosMin), Math.abs(hFFPosMax - hFFPos)); 
			if (inaccuracy <= THRESHOLD ) { hSearch.value = false; hFFLost.value = false;
			} else { 						hSearch.value = true;  hFFLost.value = true;  hFFConnected.value = false;}
			hFFTargetPos.value = hPos; 
			hFFTargetSpeed.value = DESIRED_SPEED;

			if(!hFFConnected.value && inaccuracy >= 0 && hFFLost.value){
				if(hHSearch.value || hSearch.value){
