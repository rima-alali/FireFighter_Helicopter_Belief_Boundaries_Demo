...
public class Helicopter1 extends Component {
	...
	public Boolean hHFFConnected = false;
	...
	public static void computeTarget(
			...
			@InOut("hSearch") OutWrapper<Boolean> hSearch,
			...
			@InOut("hHFFConnected") OutWrapper<Boolean> hHFFConnected
			) {
		...
		if(hFFCreationTime.value != 0.0){
			...
			if (hFFTargetPos.value != 0.0) inaccuracy = Math.max( Math.abs(hFFPos.value  - hFFPosMin), Math.abs(hFFPosMax - hFFPos.value)); 
			...
			if(!hFFConnected.value && inaccuracy >= 0 && hHConnected){
				if(hHPos != 0.0) computeBeliefBoundariesHelicopter(hHPos, hHSpeed, hHPos, hHCreationTime);
				hSearch.value = hasToMove(hPos,hHPos,hRangeDistance);
				
				if(hSearch.value && !hHFFConnected.value){
					if(       hFFPosMin > hPos){ hFFTargetPos.value = hFFPosMax;           hFFTargetSpeed.value = hFFSpeedMax;
					} else if(hPos > hFFPosMax){ hFFTargetPos.value = hPos+hRangeDistance; hFFTargetSpeed.value = DESIRED_SPEED;
					} else {					 hFFTargetPos.value = hFFPosMax;	       hFFTargetSpeed.value = hFFSpeedMax;   }											
	...
	private static void computeBeliefBoundaries( Double hFFPos, Double hFFSpeed, Double hFFTargetPos, Double hFFCreationTime ) {
		...
		if(hFFTargetPos != 0.0 ) {
			...
			double accMin = Database.getAcceleration(hFFSpeedMin, hFFPosMin, Database.lTorques, 0.0, 1.0, Database.lMass);
			double accMax = Database.getAcceleration(hFFSpeedMax, hFFPosMax, Database.lTorques, 1.0, 0.0, Database.lMass);
	...
	private static void computeBeliefBoundariesHelicopter( Double hHPos, Double hHSpeed, Double hHTargetPos, Double hHCreationTime ) {
			...
			if(hHSpeedMax > 500 ) hHSpeedMax = 500.0;
	...
	private static Boolean hasToMove(double hPos, double hHPos, double hRangeDistance) {
		...
		if(hFFPosMin > hPos){       distH1 = hFFPosMax - hPos;
		}else if(hPos > hFFPosMax){	distH1 = hPos - hFFPosMin;
		}else {						distH1 = Math.max(hFFPosMax - hPos, hPos - hFFPosMin); }
		//----------------------------------------- distance between Helicopter2 and FireFighter ---------------------------------------------------------------
		if (hHPosMin > hFFPosMax ){	    distH2 = hHPosMax - hFFPosMin;
		}else if(hFFPosMin > hHPosMax){	distH2 = hFFPosMax - hHPosMin;
		}else{							distH2 = Math.max(hHPosMax - hFFPosMin, hFFPosMax - hHPosMin); }
		//------------------------------------------ compare between two distances ---------------------------------------------------------------
		if(hHPos == 0.0){ result = true;
		}else if((distH1 < distH2) && hPos != 0.0 && hHPos != 0.0 && distH1 < 2*hRangeDistance ){ result = true;
		}else if((distH1 > distH2)&& hPos != 0.0 && hHPos != 0.0 && distH2 < 2*hRangeDistance ){	result = false;
		}else if((distH1 < distH2)&& hPos != 0.0 && hHPos != 0.0){	result = true;
		}else result = false;
		//---------------------------------------------------------------------------------------------------------
		return result;
	}
